	...
	BEGIN
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS cst_marital_status, --Normalize marital status values to a readable format
			CASE
		...
		end_time :=NOW();
		RAISE NOTICE '>> silver.crm_cust_info rows loaded: %', (SELECT COUNT(*) FROM silver.crm_cust_info);
		...
		end_time :=NOW();
		RAISE NOTICE '>>  silver.crm_prd_info rows loaded: %', (SELECT COUNT(*) FROM silver.crm_prd_info);
		RAISE NOTICE '>> Load Duration % seconds', EXTRACT(EPOCH FROM end_time - start_time);
		RAISE NOTICE'---------------------------------------------------------------';
		------------------------------------------------------------------------------------------------------------------------------------------------
		start_time :=NOW();
		...
		end_time :=NOW();
		RAISE NOTICE '>> silver.crm_sales_details rows loaded: %', (SELECT COUNT(*) FROM silver.crm_sales_details);
		RAISE NOTICE '>> Load Duration % seconds', EXTRACT(EPOCH FROM end_time - start_time);
		RAISE NOTICE'---------------------------------------------------------------';

	EXCEPTION
		WHEN OTHERS THEN
			RAISE NOTICE 'ERROR LOADING SILVER CRM TABLES: %', SQLERRM;
	END;
-------------------------------------------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------------------------------------------
	BEGIN
		...
		end_time :=NOW();
		RAISE NOTICE '>> silver.erp_cust_az12 rows loaded: %', (SELECT COUNT(*) FROM silver.erp_cust_az12);
		RAISE NOTICE '>> Load Duration % seconds', EXTRACT(EPOCH FROM end_time - start_time);
		RAISE NOTICE'---------------------------------------------------------------';
		-------------------------------------------------------------------------------------------------------------------------------------
		start_time :=NOW();
		...
		end_time :=NOW();
		RAISE NOTICE '>> silver.erp_loc_a101 rows loaded: %', (SELECT COUNT(*) FROM silver.erp_loc_a101);
		RAISE NOTICE '>> Load Duration % seconds', EXTRACT(EPOCH FROM end_time - start_time);
		RAISE NOTICE'---------------------------------------------------------------';
		-------------------------------------------------------------------------------------------------------------------------------------
		start_time :=NOW();
		...
		end_time :=NOW();
		RAISE NOTICE '>> silver.erp_px_cat_g1v2 rows loaded: %', (SELECT COUNT(*) FROM silver.erp_px_cat_g1v2);
		RAISE NOTICE '>> Load Duration % seconds', EXTRACT(EPOCH FROM end_time - start_time);

	EXCEPTION
		WHEN OTHERS THEN
			RAISE NOTICE 'ERROR LOADING SILVER ERP TABLES: %', SQLERRM;
	END;
-----------------------------------------------------------------------------------------------------------------------
	batch_end :=NOW();
	RAISE NOTICE '=============================================================';
    RAISE NOTICE 'Total Silver Layer Load Duration: % seconds',
